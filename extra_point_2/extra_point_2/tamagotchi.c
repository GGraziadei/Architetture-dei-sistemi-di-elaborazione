...
void tamagotchi_life_interrupt ( session_t* session ) {
	...
	else if ( session->animation.animation_version != EATING && session->animation.animation_version != CUDDLES ){
...
void tamagotchi_eat_interrupt(session_t * session){
	...
	if ( session->in_motion < IN_MOTION_NUM )
	{
		...
		reset_timer(TAMAGOTCHI_TIMER_CUSTOM_ANIMATION_NUM);
...
Cruddle interraput
*/
void tamagotchi_cuddle_interrupt(session_t * session){
	session->animation.animation_version = CUDDLES;
	
	if ( session->in_motion < CUDDLES_FRAME_N )
	{
		draw_cuddles(session->in_motion-1, session->in_motion);
		session->in_motion += 1;
		return;
	}
	else {
...
void tamagotchi_happiness_update( session_t* session , int update ){
	...
	if( update + session->status.happiness > SESSION_HAPPINESS_INIT ) {
		return;
	}
...
void tamagotchi_user_wrapper_update( session_t* session , event_type event ){
	...
	print_lcd_from_FRAME(0, TAMAGOTCHI_SQUARE_Y1 + (TAMAGOTCHI_SQUARE_Y_LEN - IN_MOTION_Y ) / 2 , 0);
	session->in_motion += 1 ;
	play_wrapper(&session->sound);
}

void tamagotchi_user_wrapper_cuddle( session_t* session){
	note_t note;
	...
	user_events_disable();
	init_timer (TAMAGOTCHI_TIMER_CUSTOM_ANIMATION_NUM , TAMAGOTCHI_TIMER_CUSTOM_CRUDDLE_TC );
	clear_tamagotchi_window();
	session->in_motion += 1;
	enable_timer(TAMAGOTCHI_TIMER_CUSTOM_ANIMATION_NUM);
	draw_cuddles(-1,session->in_motion-1 );
	play_wrapper(&session->sound);
}
...
void tamagotchi_satiety_update ( session_t* session , int update ){
	...
	if ( update == 0 || update + session->status.satiety > SESSION_HAPPINESS_INIT  ) { return; }	
	...
	//No rescale update if custom animation
