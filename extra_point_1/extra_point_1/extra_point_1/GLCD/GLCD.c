...
void LCD_DrawRectangle_border (uint16_t X_start, uint16_t Y_start, uint16_t width, uint16_t height, uint16_t color , uint16_t border_color , uint16_t border_radius)
{
	uint16_t i, j;
	...
	for (i = 0; i < width; i++) {
		for (j = 0; j < height; j++) {
			if( i <= border_radius || j <= border_radius ){
				LCD_SetPoint(X_start + i, Y_start + j, border_color);
			}
			else {
				LCD_SetPoint(X_start + i, Y_start + j, color);
			}
...
void LCD_DrawBorder (uint16_t X_start, uint16_t Y_start, uint16_t width, uint16_t height, uint16_t color, uint16_t border_radius){
	uint16_t i, j;
	
	
	for (i = 0; i < width; i++) 
		for (j = 0; j < height; j++)
			if(i <= border_radius || j <= border_radius  || (width-i) <= border_radius || (height-j) <= border_radius )
				LCD_SetPoint(X_start + i, Y_start + j, color);
}
